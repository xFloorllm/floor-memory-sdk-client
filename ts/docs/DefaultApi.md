...
> object apiDeveloperCreateAppPost(inputInfo, icon)
...
> object apiDeveloperModifyAppPost(inputInfo, file)
...
import type { ApiDeveloperModifyAppPostRequest } from '@xfloor/floor-memory-sdk-ts';
...
async function example() {
  ...
  const body = {
    ...
    inputInfo: inputInfo_example,
    // Blob | Upload the new PNG icon file. (optional)
    file: BINARY_DATA_HERE,
  } satisfies ApiDeveloperModifyAppPostRequest;

  try {
    const data = await api.apiDeveloperModifyAppPost(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| Name | Type | Description | Notes |
|------------- | ------------- | ------------- | -------------|
| **inputInfo** | `string` | User ID, App ID, Title, Details. User ID and App ID are required parameters. Title/details are optional. | [Defaults to `undefined`] |
| **file** | `Blob` | Upload the new PNG icon file. | [Optional] [Defaults to `undefined`] |
...
import type { ConversationThreadsRequest } from '@xfloor/floor-memory-sdk-ts';
...
async function example() {
  ...
  } satisfies ConversationThreadsRequest;

  try {
    const data = await api.conversationThreads(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
> GetConversations200Response getConversations(userId, threadId)
...
import type { GetConversationsRequest } from '@xfloor/floor-memory-sdk-ts';
...
import type { SendSignInValidationCodeRequest } from '@xfloor/floor-memory-sdk-ts';
...
async function example() {
  ...
  const body = {
    ...
    // string | Email ID (optional)
    emailId: emailId_example,
  } satisfies SendSignInValidationCodeRequest;

  try {
    const data = await api.sendSignInValidationCode(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
