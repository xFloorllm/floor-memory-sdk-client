...
import type { GetRecentEventsRequest } from '@xfloor/floor-memory-sdk-ts';
...
async function example() {
  ...
  const api = new GetRecentEventsApi(config);
  ...
  try {
    const data = await api.getRecentEvents(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
|------------- | ------------- | ------------- | -------------|
| **floorId** | `string` |  | [Defaults to `undefined`] |
